 ...
 * 
 * Artistic brush strokes with calligraphy effects
 */
...
function draw(ctx, width, height, params, time, utils) {
  ...
  const scaledWeight = p.strokeWeight * baseScale;

  // Apply paper texture background
  if (p.paperTexture > 0) {
    applyPaperTexture(ctx, width, height, p.paperTexture);
  }

  // Draw multiple strokes with layering
  for (let s = 0; s < strokeCount; s++) {
    const strokePhase = (s / strokeCount) * Math.PI * 2;
    const strokeOffset = time * p.frequency + strokePhase;
    const layerOpacity = 1 - (s * p.layering * 0.2);
    
    // Generate stroke path
    const strokePath = generateArtisticStroke(
      centerX, centerY, scaledAmplitude, 
      p.complexity, strokeOffset, s
    );

    // Apply stroke style
    ctx.save();
    ctx.globalAlpha = layerOpacity * p.strokeOpacity;
    
    switch (strokeStyleNum) {
      case 0: // Ink Pen
        drawInkPenStroke(ctx, strokePath, scaledWeight, p.strokeVariation, p.strokeTaper, p.inkFlow, p.fillColor);
        break;
      case 1: // Brush
        drawBrushStroke(ctx, strokePath, scaledWeight, p.strokeVariation, p.strokeTexture, p.inkFlow, p.fillColor);
        break;
      case 2: // Calligraphy
        drawCalligraphyStroke(ctx, strokePath, scaledWeight, p.strokeVariation, p.strokeTaper, p.fillColor);
        break;
      case 3: // Pencil
        drawPencilStroke(ctx, strokePath, scaledWeight, p.strokeTexture, p.fillColor);
        break;
      case 4: // Marker
        drawMarkerStroke(ctx, strokePath, scaledWeight, p.inkFlow, p.fillColor);
        break;
    }
  ...
  // Add artistic flair
  if (p.artisticFlair > 0) {
    addArtisticFlair(ctx, centerX, centerY, scaledAmplitude, p.artisticFlair, p.fillColor);
  }

  // Add ink splatters
  if (p.inkSplatter > 0) {
    addInkSplatter(ctx, width, height, p.inkSplatter, p.fillColor);
  }
