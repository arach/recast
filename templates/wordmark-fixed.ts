...
const metadata = {
  id: 'wordmark-fixed',
  name: "📝 Wordmark Fixed",
  description: "Professional text-based logos with enhanced opacity and frame support",
  parameters,
  defaultParams: {
    text: 'BRAND',
    fontStyle: 'modern',
...
function drawVisualization(ctx: CanvasRenderingContext2D, width: number, height: number, params: any, time: number, utils: TemplateUtils) {
  ...
  if (showFrame) {
    ...
    // Draw based on frame style - improved opacity handling
    if (frameStyle === 'filled' || frameStyle === 'filled-inverse') {
      // Filled rectangle with proper opacity
      ctx.globalAlpha = frameStyle === 'filled' ? strokeOpacity : fillOpacity;
