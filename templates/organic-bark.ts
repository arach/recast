...
const parameters = {
  frequency: {
    default: 0.6,
    range: [0.2, 1.5, 0.05]  // min, max, step
  },
  amplitude: {
    default: 140,
    range: [80, 200, 5]
  ...
  barkType: {
    default: 1,
    range: [0, 4, 1]  // 0=Oak, 1=Pine, 2=Birch, 3=Redwood, 4=Ancient
  },
  growthComplexity: {
    default: 0.7,
    range: [0.3, 1, 0.05]
...
function drawVisualization(ctx: CanvasRenderingContext2D, width: number, height: number, params: any, time: number, utils: TemplateUtils) {
  ...
  function organicNoise(x: number, y: number, scale: number, octaves: number = 3): number {
    let value = 0;
    let amplitude = 1;
    let frequency = scale;
    let maxValue = 0;
