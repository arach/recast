...
export const parameters: Record<string, ParameterDefinition> = {
  // Core constellation parameters
  frequency: { type: 'slider', min: 0.1, max: 4, step: 0.1, default: 1.2, label: 'Formation Speed' },
  amplitude: { type: 'slider', min: 30, max: 150, step: 5, default: 80, label: 'Constellation Size' },
  complexity: { type: 'slider', min: 0, max: 1, step: 0.01, default: 0.7, label: 'Connection Density' },
  chaos: { type: 'slider', min: 0, max: 1, step: 0.01, default: 0.15, label: 'Node Variance' },
  damping: { type: 'slider', min: 0.3, max: 1, step: 0.01, default: 0.8, label: 'Energy Decay' },
  layers: { type: 'slider', min: 1, max: 5, step: 1, default: 2, label: 'Network Layers' },
  
  // Brand shape parameters  
  shapeType: { 
    type: 'slider', 
    min: 0, 
    max: 5, 
    step: 1, 
    default: 0, 
    label: 'Brand Shape (0=Circle, 1=Star, 2=Shield, 3=Hex, 4=Diamond, 5=Triangle)' 
  },
  nodeCount: { type: 'slider', min: 3, max: 24, step: 1, default: 8, label: 'Network Nodes' },
  connectionStyle: {
    type: 'slider',
    min: 0,
    max: 3,
    step: 1,
    default: 0,
    label: 'Connection Style (0=Flowing, 1=Neural, 2=Magnetic, 3=Minimal)'
  },
  energyFlow: { type: 'slider', min: 0, max: 1, step: 0.01, default: 0.8, label: 'Energy Flow' },
  glowIntensity: { type: 'slider', min: 0, max: 1, step: 0.05, default: 0.6, label: 'Glow Effect' }
};
...
export function draw(
  ...
  function generateBrandShapeNodes(shapeType: string, centerX: number, centerY: number, radius: number, nodeCount: number, phase: number, chaos: number, random: () => number) {
    const nodes: Array<{x: number, y: number, energy: number}> = [];
  ...
  function drawConnections(ctx: CanvasRenderingContext2D, nodes: any[], style: string, complexity: number, energyFlow: number, time: number, phase: number, hue: number, saturation: number, lightness: number, alpha: number, glowIntensity: number) {
  ...
  function drawNodes(ctx: CanvasRenderingContext2D, nodes: any[], scale: number, time: number, phase: number, hue: number, saturation: number, lightness: number, alpha: number, glowIntensity: number) {
    ctx.save();
  ...
  function drawNetworkParticles(ctx: CanvasRenderingContext2D, centerX: number, centerY: number, radius: number, time: number, complexity: number) {
    ctx.save();
